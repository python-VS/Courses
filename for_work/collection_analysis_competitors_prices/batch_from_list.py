...
def pars_data(x):
    ...
    res = res.assign(РазнШатАрм=res['Цена\nАРМТЕК\nEUR'] / res['ОПТ РБ'] - 1).replace([-np.inf, np.inf], ['', ''])  # добавление столбца для определения разницы цен с Армтек + исправление Inf на пустоые при делении на 0
    res = res.merge(mot_1, how='left', left_on='Номер1', right_on='Номер1')  # добавление цен Мотехс
    res = res.assign(РазнШатМот=res['Цена\nМОТЕХС\nEUR'] / res['ОПТ РБ'] - 1).replace([-np.inf, np.inf], ['', ''])  # добавление столбца для определения разницы цен с Мотехс + исправление Inf на пустоые при делении на 0
    ...
    res = res.assign(РазнШатМИН=res['MIN\nЦена\nКонк'] / res['ОПТ РБ'] - 1).replace([-np.inf, np.inf], ['', ''])  # добавление столбца для определения разницы цен с МИН КОНК + исправление Inf на пустоые при делении на 0
    res = res.assign(МНподМИНконк=res['MIN\nЦена\nКонк'] / res['Себест ед.'] - 1).replace([-np.inf, np.inf], ['', ''])  # добавление столбца с расчетом МН под МИН цены конк + исправление Inf на пустоые при делении на 0
    ...
    res = res.assign(Изменение=res['Нов\nОПТ'] / res['ОПТ РБ'] - 1).replace([-np.inf, np.inf], ['', ''])  # добавление столбца с расчетом разницы новый ОПТ/старый ОПТ + исправление Inf на пустоые при делении на 0
    res = res.assign(НоваяМН=res['Нов\nОПТ'] / res['Себест ед.'] - 1).replace([-np.inf, np.inf], ['', ''])  # добавление столбца с расчетом МН под новый ОПТ + исправление Inf на пустоые при делении на 0
    ...
    res = res.assign(ТекМН_прод=(res['ОПТ РБ'] * res['СуммаПродаж']) / (res['Себест ед.'] * res['СуммаПродаж']) - 1).replace([-np.inf, np.inf], ['', ''])  # добавление столбца с расчетом МН под тек ОПТ с учетом продаж + исправление Inf на пустоые при делении на 0
    res = res.assign(БудМН_прод=(res['Нов\nОПТ'] * res['СуммаПродаж']) / (res['Себест ед.'] * res['СуммаПродаж']) - 1).replace([-np.inf, np.inf], ['', ''])  # добавление столбца с расчетом МН под нов ОПТ с учетом продаж + исправление Inf на пустоые при делении на 0
    res = res.assign(РазнТекАрмШат=(res['Цена\nАРМТЕК\nEUR'] * res['СуммаПродаж']) / (res['ОПТ РБ'] * res['СуммаПродаж']) - 1).replace([-np.inf, np.inf], ['', ''])  # добавление столбца с расчетом МН под тек ОПТ с учетом продаж + исправление Inf на пустоые при делении на 0
    res = res.assign(РазнБудАрмШат=(res['Цена\nАРМТЕК\nEUR'] * res['СуммаПродаж']) / (res['Нов\nОПТ'] * res['СуммаПродаж']) - 1).replace([-np.inf, np.inf], ['', ''])  # добавление столбца с расчетом МН под нов ОПТ с учетом продаж + исправление Inf на пустоые при делении на 0
    ...
    with pd.ExcelWriter(f'Анализ_{tm1}_{current_date}.xlsx', engine='xlsxwriter') as wb:
        wb.book.set_properties({'author': 'Saharov', 'company': 'Shate-M Plus'})
        res.to_excel(wb, sheet_name=f'анализ_{tm1}', freeze_panes=(2, 2), index=False,
                     startrow=1)  # сохранение в файла [freeze_panes=(row, col), startrow(с какой строки сверху вставлять ДФ)
        ...
        cell_format = wb.book.add_format(
            {'bold': 0, 'font_color': 'black', 'bg_color': '#FFFFFF', 'font_name': 'Calibri', 'font_size': 8,
             'border': 0})

        # настройка индивидуальных форматов столбцов
        cell_format_opt = wb.book.add_format(
            {'bold': 0, 'font_color': 'black', 'bg_color': '#32CD32', 'font_size': 9})  # цвет ячеек салатовый
        cell_format_new_opt = wb.book.add_format(
            {'bold': 1, 'font_color': 'black', 'bg_color': '#FFFF00', 'font_size': 9})  # цвет ячеек желтый
        cell_format_cost = wb.book.add_format(
            {'bold': 0, 'font_color': 'black', 'bg_color': '#C0C0C0', 'font_size': 9})  # цвет ячеек серый
        cell_format_perc = wb.book.add_format({"num_format": "0.0%", 'border': 0, 'font_size': 8})
        cell_format_min = wb.book.add_format(
            {'bold': 1, 'font_color': 'black', 'bg_color': '#1E90FF', 'font_size': 9})  # цвет ячеек голубой

        cell_format_condit1 = wb.book.add_format({'bold': 0, 'italic': 0, 'bg_color': '#FFA07A',
                                                  'border': 0})  # шаблон условного форматирования (красный цвет ячейки)
        cell_format_condit2 = wb.book.add_format({'bold': 0, 'italic': 0, 'bg_color': '#00FF00',
                                                  'border': 0})  # шаблон условного форматирования (зеленый цвет ячейки)
        cell_format_condit3 = wb.book.add_format({'bold': 0, 'italic': 0, 'bg_color': '#FFFFFF',
                                                  'border': 0})  # шаблон условного форматирования (белый цвет ячейки)
        merge_format = wb.book.add_format(
            {'bold': True, 'border': 1, 'align': 'center', 'valign': 'vcenter', 'fg_color': '#9370DB', 'font_size': 10})

        sheet.set_column(0, 32, 10,
                         cell_format)  # Установка стиля для 20 первых столбов (1й столб, посл столб, ширина, формат)
        sheet.set_column(6, 6, 8,
                         cell_format_opt)  # Установка стиля для 7 столба (1й столб, посл столб, ширина, формат)
        sheet.set_column(10, 10, 8,
                         cell_format_cost)  # Установка стиля для 11 столба (1й столб, посл столб, ширина, формат)
        sheet.set_column(20, 20, 8,
                         cell_format_new_opt)  # Установка стиля для 11 столба (1й столб, посл столб, ширина, формат)
        sheet.set_column(17, 17, 8,
                         cell_format_min)  # Установка стиля для 18 столба (1й столб, посл столб, ширина, формат)
        ...
        sheet.conditional_format('O2:O5000', {'type': 'cell', 'criteria': '<', 'value': 0,
                                              'format': cell_format_condit1})  # условное форматирование столба O "если меньше 0, то красный цвет"
        sheet.conditional_format('O2:O5000', {'type': 'cell', 'criteria': '>', 'value': 0,
                                              'format': cell_format_condit2})  # условное форматирование столба O "если больше 0, то зеленый цвет"
        sheet.conditional_format('O2:O5000', {'type': 'blanks',
                                              'format': cell_format_condit3})  # условное форматирование столба O "если пусто, то белый цвет"

        sheet.conditional_format('Q2:Q5000', {'type': 'cell', 'criteria': '<', 'value': 0,
                                              'format': cell_format_condit1})  # условное форматирование столба Q "если меньше 0, то красный цвет"
        sheet.conditional_format('Q2:Q5000', {'type': 'cell', 'criteria': '>', 'value': 0,
                                              'format': cell_format_condit2})  # условное форматирование столба Q "если больше 0, то зеленый цвет"
        sheet.conditional_format('Q2:Q5000', {'type': 'blanks',
                                              'format': cell_format_condit3})  # условное форматирование столба Q "если пусто, то белый цвет"

        sheet.conditional_format('S2:S5000', {'type': 'cell', 'criteria': '<', 'value': 0,
                                              'format': cell_format_condit1})  # условное форматирование столба S "если меньше 0, то красный цвет"
        sheet.conditional_format('S2:S5000', {'type': 'cell', 'criteria': '>', 'value': 0,
                                              'format': cell_format_condit2})  # условное форматирование столба S "если больше 0, то зеленый цвет"
        sheet.conditional_format('S2:S5000', {'type': 'blanks',
                                              'format': cell_format_condit3})  # условное форматирование столба Q "если пусто, то белый цвет"
